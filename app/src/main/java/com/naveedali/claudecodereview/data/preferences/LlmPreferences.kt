package com.naveedali.claudecodereview.data.preferences
 ...
 * ─────────────────────
 * On first instantiation the class checks if OPENAI_API_KEY was baked into the
 * build via local.properties and, if so, migrates it into SharedPreferences.
 * This means a user who previously added the key to local.properties will see
 * it pre-filled in the Settings screen without any extra steps.
 *
...
class LlmPreferences(context: Context) {
    ...
    init {
        // One-time migration: if the user had OPENAI_API_KEY in local.properties
        // and the SharedPreferences key is not yet set, copy it over.
        if (getApiKey(LlmProvider.OPENAI).isBlank() && BuildConfig.OPENAI_API_KEY.isNotBlank()) {
            saveApiKey(LlmProvider.OPENAI, BuildConfig.OPENAI_API_KEY)
        }
    }
